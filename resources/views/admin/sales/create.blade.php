...
<div>
    ...
    <div class="card max-w-2xl">
        ...
        <form action="{{ route('admin.sales.store') }}" method="POST" id="saleForm">
            ...
            <!-- Поле для вибору куди йде прибуток -->
            <div class="form-group">
                <label for="profit_to_admin">Куди йдуть гроші</label>
                <select id="profit_to_admin" name="profit_to_admin">
                    <option value="0" {{ old('profit_to_admin') == '0' ? 'selected' : '' }}>На рахунок менеджера</option>
                    <option value="1" {{ old('profit_to_admin') == '1' ? 'selected' : '' }}>На рахунок адміна</option>
                </select>
            </div>
                ...
                <div class="form-group">
                    ...
                    <select id="product_id" name="product_id">
                        ...
                        @foreach($products as $product)
                            <option value="{{ $product->id }}" data-price="{{ $product->purchase_price ?? 0 }}" {{ old('product_id') == $product->id ? 'selected' : '' }}>
                                {{ $product->name }} - {{ number_format($product->purchase_price ?? 0, 2) }} zł (закуп.)
                            </option>
                        @endforeach
                    </select>
                </div>
            ...
            <div class="flex flex-col sm:flex-row gap-4 sm:gap-4 flex-wrap mt-8 pt-6 border-t border-gray-200">
                ...
                <a href="{{ route('admin.sales.index') }}" class="btn-secondary flex-1 sm:flex-none text-center">
                    Скасувати
                </a>
            </div>
        </form>
        ...
        <script>
            ...
            function addSaleItem() {
                ...
                products.forEach(product => {
                    productOptions += `<option value="${product.id}" data-price="${product.purchase_price || 0}">
                        ${product.name} - ${(product.purchase_price || 0).toFixed(2)} zł (закуп.)
                    </option>`;
                });
